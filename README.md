...
하나씩 찾아 합산하고, **반복 실험을 평균 ± SD 로 취합**하던 **수작업 전 과정을 몇 초로 대체**한다.
파일 하나는 물론 **폴더째(반복 여러 개)** 넣으면 개별 결과 + 취합 결과를 한 번에 낸다.
...
| **반복 평균 ± SD 취합** (Table 1 형식) | 폴더 배치 → `aggregated.xlsx` 자동 | 동일 반복 SD=0 / 산발검출 SD로 표기 |
...
| 옵션 | 뜻 | 기본 |
|---|---|---|
| `input` | 입력 `.raw`/`.mzML` **또는 폴더**(폴더면 배치+취합) (필수) | — |
...
glyco/                 # 라이브러리
  ...
  aggregate.py         #   반복 취합(평균±SD, 검출빈도)
  pipeline.py          #   오케스트레이션 analyze() / batch() / screening()
  report.py            #   엑셀 출력(개별 / aggregated / screening)
scripts/               # 검증 하네스(eval_ppe: 논문정답 / fp_probe: 위양성)
tests/                 # 회귀 가드레일(질량/정량/명명/설정/취합)
```
